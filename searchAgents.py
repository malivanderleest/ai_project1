        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # Here's a code snippet for figuring out whether a new position hits a wall:
                ...
                if not self.walls[nextx][nexty]:
                        ...
                        successor = ((nextState,nextCorner), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        if corner not in cornersVisited:
            toVisit.add(corner)
    while toVisit:
        cost, corner =  min([(util.manhattanDistance(position, corner), corner) for corner in toVisit])
        toVisit.remove(corner)
        position = corner
        totCost += cost

    return totCost  # Default to trivial solution
...
def foodHeuristic(state, problem):
    ...
    maxDistIndex = distances.index(maxDist)
